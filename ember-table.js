  ...
  Ember.ResizeHandler = Ember.Mixin.create({
    ...
    handleWindowResize: function(event) {
      if (!this.get('resizing')) {
        ...
        if (typeof this.onResizeStart === "function") this.onResizeStart(event);
      } else {
        if (typeof this.onResize === "function") this.onResize(event);
      }
...
;(function() {
  ...
  Ember.StyleBindingsMixin = Ember.Mixin.create({
    ...
    createStyleString: function(styleName, property) {
      ...
      if (Ember.typeOf(value) === 'number') value = value + this.get('unitType');
...
;(function() {
  ...
  Ember.LazyContainerView = Ember.ContainerView.extend(Ember.StyleBindingsMixin, {
    ...
    onNumChildViewsDidChange: Ember.observer(function() {
      ...
      if (!(itemViewClass && newNumViews)) return;
    ...
    viewportDidChange: Ember.observer(function() {
      var content, numChildViews, numShownViews, startIndex, views, _i, _j, _results, _results2,
        _this = this;
      ...
      return (function() {
        _results2 = [];
        for (var _j = numShownViews; numShownViews <= numChildViews ? _j < numChildViews : _j > numChildViews; numShownViews <= numChildViews ? _j++ : _j--){ _results2.push(_j); }
        return _results2;
      }).apply(this).forEach(function(i) {
        var childView;
  ...
  Ember.LazyItemView = Ember.View.extend(Ember.StyleBindingsMixin, {
    ...
    display: Ember.computed(function() {
      if (!this.get('content')) return 'none';
    }).property('content')
  });
...
;(function() {
  ...
  Ember.MouseWheelHandlerMixin = Ember.Mixin.create({
    ...
    willDestroy: function() {
      var _ref;
      if ((_ref = this.$()) != null) _ref.unbind('mousewheel');
      return this._super();
    }
  });
  ...
  Ember.ScrollHandlerMixin = Ember.Mixin.create({
    ...
    willDestroy: function() {
      var _ref;
      if ((_ref = this.$()) != null) _ref.unbind('scroll');
      return this._super();
    }
  });
...
;(function() {
  ...
  Ember.Table.TableController = Ember.Controller.extend({
    ...
    _rowWidth: Ember.computed(function() {
      ...
      if (columnsWidth < nonFixedTableWidth) return nonFixedTableWidth;
      return columnsWidth;
    ...
    _bodyHeight: Ember.computed(function() {
      ...
      if (this.get('hasHeader')) bodyHeight -= headerHeight;
      if (this.get('hasFooter')) bodyHeight -= footerHeight;
      return bodyHeight;
    ...
    _startIndex: Ember.computed(function() {
      ...
      if (index + numViews >= numContent) index = numContent - numViews;
      if (index < 0) {
        return 0;
      } else {
        return index;
...
;(function() {
  ...
  Ember.Table.RowSelectionMixin = Ember.Mixin.create({
    ...
    mouseDown: function(event) {
      ...
      if (sel.contains(index) && sel.length === 1) return sel.clear();
    ...
    setSelectionIndex: function(index) {
      var sel;
      if (!this.ensureIndex(index)) return;
      sel = this.get('selectionIndices');
  ...
  Ember.Table.RowMultiSelectionMixin = Ember.Mixin.create(Ember.Table.RowSelectionMixin, {
    ...
    expandSelectionRange: function(indices) {
      ...
      _ref = [_.min(indices), _.max(indices)], min = _ref[0], max = _ref[1];
      if (!range) {
        range = {
          min: min,
          max: max
    ...
    mouseDown: function(event) {
      ...
      if (event.ctrlKey || event.metaKey) {
        ...
        range = this.get('selectionRange');
        if (range) return this.setSelectionRange(range.min, index, index);
      } else {
    ...
    upArrowPressed: function(event) {
      ...
      if (event.shiftKey) {
        range = this.get('selectionRange');
        index = range.min - 1;
        if (range) return this.setSelectionRange(index, range.max, index);
      } else {
        return this._super(event);
      }
    },
    ...
    setSelectionRange: function(start, end, visibleIndex) {
      ...
      if (!(this.ensureIndex(start) && this.ensureIndex(end))) return;
...
;(function() {
  ...
  Ember.Table.TableRow = Ember.LazyItemView.extend({
    ...
    mouseEnter: function(event) {
      var row;
      row = this.get('row');
      if (row) return row.set('active', true);
    },
    mouseLeave: function(event) {
      var row;
      row = this.get('row');
      if (row) return row.set('active', false);
    },
