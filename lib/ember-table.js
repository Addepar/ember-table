...
function program3(depth0,data) {
  ...
  data.buffer.push("\n");
  hashTypes = {};
  data.buffer.push(escapeExpression(helpers.view.call(depth0, "Ember.Table.ColumnSortableIndicator", {hash:{},contexts:[depth0],types:["ID"],hashTypes:hashTypes,data:data})));
  data.buffer.push("\n");
  hashTypes = {};
  data.buffer.push(escapeExpression(helpers.view.call(depth0, "Ember.Table.AddColumnButton", {hash:{},contexts:[depth0],types:["ID"],hashTypes:hashTypes,data:data})));
  data.buffer.push("\n");
  return buffer;
...
Ember.Table.TableController = Ember.Controller.extend({
  ...
  _hasVerticalScrollbar: Ember.computed(function() {
    ...
    contentHeight = this.get('_tableContentHeight') + this.get('headerHeight') + this.get('footerHeight');
  ...
  _tablesContainerHeight: Ember.computed(function() {
    ...
    if (this.get('_hasHorizontalScrollbar')) {
      height += this.get('_scrollbarSize');
    }
    return height;
  }).property('_height', '_tableContentHeight', 'headerHeight', 'footerHeight', '_hasHorizontalScrollbar', '_scrollbarSize'),
  ...
  _bodyHeight: Ember.computed(function() {
    ...
    if (this.get('hasHeader')) {
      bodyHeight -= this.get('headerHeight');
    }
    if (this.get('hasFooter')) {
      bodyHeight -= this.get('footerHeight');
    }
    return bodyHeight;
  }).property('_tablesContainerHeight', 'headerHeight', 'footerHeight', 'hasHeader', 'hasFooter', '_tableColumnsWidth', '_hasHorizontalScrollbar', '_scrollbarSize'),
  ...
  _scrollContainerHeight: Ember.computed(function() {
    var containerHeight;
    return containerHeight = this.get('_tablesContainerHeight') - this.get('headerHeight');
...
Ember.Table.TableRow = Ember.LazyItemView.extend({
  ...
  classNameBindings: ['row.isActive:active', 'row.isSelected:selected', 'row.rowStyle', 'isLastRow:last-row'],
  styleBindings: ['width', 'height'],
...
Ember.Table.HeaderRow = Ember.View.extend(Ember.ScrollHandlerMixin, {
  ...
  onColumnSortChange: function(event, ui) {
    var left;
    left = this.$('.ui-state-highlight').offset().left - this.$().closest('.tables-container').offset().left;
...
Ember.Table.AddColumnButton = Ember.View.extend(Ember.StyleBindingsMixin, {
  tagName: 'span',
  template: Ember.Handlebars.compile('<a href="#">+</a>'),
  styleBindings: ['height', 'width'],
  classNames: 'add-column-button',
  height: Ember.computed(function() {
    return this.get('controller.headerHeight') + 1;
...
Ember.Table.HeaderTableContainer = Ember.Table.TableContainer.extend({
  ...
  didInsertElement: function() {
    if (this.get('controller.isHeaderHeightResizable')) {
      return this.$().resizable(this.get('resizableOption'));
    }
  },
  onColumnResize: function(event, ui) {
    return this.set('controller.headerHeight', ui.size.height);
  }
});
