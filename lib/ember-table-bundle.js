        ...
        Ember.MultiItemViewCollectionView = Ember.CollectionView.extend({
            ...
            didInsertElement: function() {
                ...
                return this._super(), startX = startY = 0, this.$().bind("touchstart", function(event) {
                    return startX = event.originalEvent.targetTouches[0].pageX, startY = event.originalEvent.targetTouches[0].pageY;
                }), this.$().bind("touchmove", function(event) {
                    var deltaX, deltaY, newX, newY;
                    return newX = event.originalEvent.targetTouches[0].pageX, newY = event.originalEvent.targetTouches[0].pageY, 
    ...
    9: [ function() {
        Ember.Table.TablesContainer = Ember.View.extend(Ember.ResizeHandler, {
            ...
            onTouchMove: function(event, deltaX, deltaY) {
                var scrollLeft;
                if (Math.abs(deltaX) > Math.abs(deltaY)) return scrollLeft = this.$(".right-table-block").scrollLeft() + deltaX, 
                this.set("scrollLeft", scrollLeft), event.preventDefault();
            }
